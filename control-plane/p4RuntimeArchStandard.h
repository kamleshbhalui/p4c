...
enum class Arch { V1MODEL, PSA, PNA, V1MODEL2020, DPDK_PNA, DPDK_PSA };
...
/// @ref CounterlikeTraits<> specialization for @ref CounterExtern for DPDK_PSA
template<> struct CounterlikeTraits<Standard::CounterExtern<Standard::Arch::DPDK_PSA> > {
    static const cstring name() { return "counter"; }
    static const cstring directPropertyName() {
        return "psa_direct_counter";
    }
...
/// @ref CounterlikeTraits<> specialization for @ref CounterExtern for DPDK_PNA
template<> struct CounterlikeTraits<Standard::CounterExtern<Standard::Arch::DPDK_PNA> > {
    static const cstring name() { return "counter"; }
    static const cstring directPropertyName() {
        return "pna_direct_counter";
    }
...
/// @ref CounterlikeTraits<> specialization for @ref MeterExtern for DPDK_PSA
template<> struct CounterlikeTraits<Standard::MeterExtern<Standard::Arch::DPDK_PSA> > {
    static const cstring name() { return "meter"; }
    static const cstring directPropertyName() {
        return "psa_direct_meter";
    }
...
/// @ref CounterlikeTraits<> specialization for @ref MeterExtern for DPDK_PNA
template<> struct CounterlikeTraits<Standard::MeterExtern<Standard::Arch::DPDK_PNA> > {
    static const cstring name() { return "meter"; }
    static const cstring directPropertyName() {
        return "pna_direct_meter";
    }
...
/// The architecture handler builder implementation for DPDK PSA.
struct DPDKPSAArchHandlerBuilder : public P4RuntimeArchHandlerBuilderIface {
    P4RuntimeArchHandlerIface* operator()(
        ReferenceMap* refMap,
        TypeMap* typeMap,
        const IR::ToplevelBlock* evaluatedProgram) const override;
...
/// The architecture handler builder implementation for DPDK PSA.
struct DPDKPNAArchHandlerBuilder : public P4RuntimeArchHandlerBuilderIface {
    P4RuntimeArchHandlerIface* operator()(
        ReferenceMap* refMap,
        TypeMap* typeMap,
        const IR::ToplevelBlock* evaluatedProgram) const override;
...
template<> struct ActionProfileTraits<Arch::DPDK_PSA> : public ActionProfileTraits<Arch::PSA> {
};
...
template<> struct ActionProfileTraits<Arch::DPDK_PNA> : public ActionProfileTraits<Arch::PNA>{
};
...
template<> struct ActionSelectorTraits<Arch::DPDK_PSA> : public ActionSelectorTraits<Arch::PSA> {
};
...
template<> struct ActionSelectorTraits<Arch::DPDK_PNA> : public ActionSelectorTraits<Arch::PNA>  {
};
...
template<> struct RegisterTraits<Arch::DPDK_PSA> : RegisterTraits<Arch::PSA> {
};
...
template<> struct RegisterTraits<Arch::DPDK_PNA> : RegisterTraits<Arch::PNA> {
};
...
/// extern type in PSA.
template <Arch arch, bool = (arch != Arch::DPDK_PNA && arch != Arch::DPDK_PSA)>
 ...
 protected:
    ...
    void addCounter(const P4RuntimeSymbolTableIface& symbols,
        ...
        if (counterInstance.table) {
            ...
            auto counter = p4Info->add_dpdk_counters();
    ...
    void addMeter(const P4RuntimeSymbolTableIface& symbols,
        ...
        if (meterInstance.table) {
            ...
            auto meter = p4Info->add_dpdk_meters();
...
/// extern type in PSA.
template <Arch arch>
class P4RuntimeArchHandlerCommon<arch, false> : public P4RuntimeArchHandlerIface {
 protected:
    using ArchCounterExtern = CounterExtern<arch>;
    using CounterTraits = Helpers::CounterlikeTraits<ArchCounterExtern>;
    using ArchMeterExtern = MeterExtern<arch>;
    ...
    using Counter = p4configv1::DPDKCounter;
    using Meter = p4configv1::DPDKMeter;
    using CounterSpec = p4configv1::CounterSpec;
    using MeterSpec = p4configv1::MeterSpec;

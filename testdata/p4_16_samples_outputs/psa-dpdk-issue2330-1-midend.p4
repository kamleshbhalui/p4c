...
control ingress(inout Headers h, inout Meta m, in psa_ingress_input_metadata_t istd, inout psa_ingress_output_metadata_t ostd) {
    ...
    @name("ingress.do_action") action do_action() {
        val1 = (h.eth_hdr.dst_addr != 48w0 ? h.eth_hdr.eth_type : val1);
        val2 = (h.eth_hdr.dst_addr != 48w0 ? h.eth_hdr.src_addr : val2);
        h.eth_hdr.eth_type = (h.eth_hdr.dst_addr != 48w0 ? val1 : h.eth_hdr.eth_type);
        h.eth_hdr.src_addr = (h.eth_hdr.dst_addr != 48w0 ? val2 : h.eth_hdr.src_addr);
...
control deparser(packet_out pkt, out EMPTY_CLONE clone_i2e_meta, out EMPTY_RESUB resubmit_meta, out EMPTY_BRIDGE normal_meta, inout Headers h, in Meta local_metadata, in psa_ingress_output_metadata_t istd) {
    @hidden action psadpdkissue23301l89() {
        pkt.emit<ethernet_t>(h.eth_hdr);
    }
    @hidden table tbl_psadpdkissue23301l89 {
        actions = {
            psadpdkissue23301l89();
        }
        const default_action = psadpdkissue23301l89();
    }
    apply {
        tbl_psadpdkissue23301l89.apply();
    }
}

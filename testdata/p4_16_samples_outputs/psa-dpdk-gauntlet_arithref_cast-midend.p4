...
control ingress(inout Headers h, inout Meta m, in psa_ingress_input_metadata_t istd, inout psa_ingress_output_metadata_t ostd) {
    @hidden action psadpdkgauntlet_arithref_cast52() {
        h.eth_hdr.eth_type = (bit<16>)(h.eth_hdr.dst_addr << 8w2);
    }
    @hidden table tbl_psadpdkgauntlet_arithref_cast52 {
        actions = {
            psadpdkgauntlet_arithref_cast52();
        }
        const default_action = psadpdkgauntlet_arithref_cast52();
    }
    apply {
        if (h.eth_hdr.src_addr < 48w10) {
            tbl_psadpdkgauntlet_arithref_cast52.apply();
        }
    }
}
...
control deparser(packet_out pkt, out EMPTY_CLONE clone_i2e_meta, out EMPTY_RESUB resubmit_meta, out EMPTY_BRIDGE normal_meta, inout Headers hdr, in Meta local_metadata, in psa_ingress_output_metadata_t istd) {
    @hidden action psadpdkgauntlet_arithref_cast75() {
        pkt.emit<ethernet_t>(hdr.eth_hdr);
    }
    @hidden table tbl_psadpdkgauntlet_arithref_cast75 {
        actions = {
            psadpdkgauntlet_arithref_cast75();
        }
        const default_action = psadpdkgauntlet_arithref_cast75();
    }
    apply {
        tbl_psadpdkgauntlet_arithref_cast75.apply();
    }
}

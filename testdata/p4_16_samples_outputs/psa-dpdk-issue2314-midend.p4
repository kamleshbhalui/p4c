...
control MyDeparser(packet_out b, out EMPTY_CLONE clone_i2e_meta, out EMPTY_RESUB resubmit_meta, out EMPTY_BRIDGE normal_meta, inout h hdr, in m local_metadata, in psa_ingress_output_metadata_t istd) {
    @hidden action psadpdkissue2314l116() {
        b.emit<ethernet_t>(hdr.ether);
        b.emit<H>(hdr.h);
        b.emit<I>(hdr.i);
    }
    @hidden table tbl_psadpdkissue2314l116 {
        actions = {
            psadpdkissue2314l116();
        }
        const default_action = psadpdkissue2314l116();
    }
    apply {
        tbl_psadpdkissue2314l116.apply();
    }
}

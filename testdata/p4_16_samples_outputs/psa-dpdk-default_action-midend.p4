...
control ingress(inout Headers h, inout Meta m, in psa_ingress_input_metadata_t istd, inout psa_ingress_output_metadata_t ostd) {
    ...
    @hidden action arithinlineskeletond58() {
        ostd.egress_port = 32w0;
    }
    @hidden table tbl_arithinlineskeletond58 {
        actions = {
            arithinlineskeletond58();
        }
        const default_action = arithinlineskeletond58();
    }
    apply {
        c_t.apply();
        tbl_arithinlineskeletond58.apply();
    }
}
...
control deparser(packet_out b, out EMPTY_CLONE clone_i2e_meta, out EMPTY_RESUB resubmit_meta, out EMPTY_BRIDGE normal_meta, inout Headers h, in Meta local_metadata, in psa_ingress_output_metadata_t istd) {
    @hidden action arithinlineskeletond77() {
        b.emit<hdr>(h.h);
    }
    @hidden table tbl_arithinlineskeletond77 {
        actions = {
            arithinlineskeletond77();
        }
        const default_action = arithinlineskeletond77();
    }
    apply {
        tbl_arithinlineskeletond77.apply();
    }
}

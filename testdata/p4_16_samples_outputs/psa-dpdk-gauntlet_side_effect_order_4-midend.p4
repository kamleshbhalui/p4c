...
control ingress(inout Headers h, inout Meta m, in psa_ingress_input_metadata_t istd, inout psa_ingress_output_metadata_t ostd) {
    ...
    @hidden action psadpdkgauntlet_side_effect_order_4l24() {
        tmp = 16w0;
    }
    @hidden action psadpdkgauntlet_side_effect_order_4l24_0() {
        tmp = 16w3;
    }
    @hidden action act() {
        h.eth_hdr.eth_type = tmp;
    }
    @hidden table tbl_psadpdkgauntlet_side_effect_order_4l24 {
        actions = {
            psadpdkgauntlet_side_effect_order_4l24();
        }
        const default_action = psadpdkgauntlet_side_effect_order_4l24();
    }
    @hidden table tbl_psadpdkgauntlet_side_effect_order_4l24_0 {
        actions = {
            psadpdkgauntlet_side_effect_order_4l24_0();
        }
        const default_action = psadpdkgauntlet_side_effect_order_4l24_0();
    }
    ...
    apply {
        if (h.eth_hdr.eth_type < 16w6) {
            tbl_psadpdkgauntlet_side_effect_order_4l24.apply();
        } else {
            tbl_psadpdkgauntlet_side_effect_order_4l24_0.apply();
        }
        tbl_act.apply();
    }
}
...
control deparser(packet_out pkt, out EMPTY_CLONE clone_i2e_meta, out EMPTY_RESUB resubmit_meta, out EMPTY_BRIDGE normal_meta, inout Headers hdr, in Meta local_metadata, in psa_ingress_output_metadata_t istd) {
    @hidden action psadpdkgauntlet_side_effect_order_4l77() {
        pkt.emit<ethernet_t>(hdr.eth_hdr);
    }
    @hidden table tbl_psadpdkgauntlet_side_effect_order_4l77 {
        actions = {
            psadpdkgauntlet_side_effect_order_4l77();
        }
        const default_action = psadpdkgauntlet_side_effect_order_4l77();
    }
    apply {
        tbl_psadpdkgauntlet_side_effect_order_4l77.apply();
    }
}

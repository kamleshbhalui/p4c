...
control MyIngressControl(inout headers_t hdr, inout user_meta_data_t m, in psa_ingress_input_metadata_t c, inout psa_ingress_output_metadata_t d) {
    ...
    @hidden action psadpdknonzeroargdefaultaction06l70() {
        d.egress_port = (bit<32>)c.ingress_port ^ 32w1;
    }
    @hidden table tbl_psadpdknonzeroargdefaultaction06l70 {
        actions = {
            psadpdknonzeroargdefaultaction06l70();
        }
        const default_action = psadpdknonzeroargdefaultaction06l70();
    }
    apply {
        tbl_psadpdknonzeroargdefaultaction06l70.apply();
        stub_0.apply();
    }
}
...
control MyIngressDeparser(packet_out pkt, out EMPTY a, out EMPTY b, out EMPTY c, inout headers_t hdr, in user_meta_data_t e, in psa_ingress_output_metadata_t f) {
    @hidden action psadpdknonzeroargdefaultaction06l85() {
        pkt.emit<ethernet_t>(hdr.ethernet);
    }
    @hidden table tbl_psadpdknonzeroargdefaultaction06l85 {
        actions = {
            psadpdknonzeroargdefaultaction06l85();
        }
        const default_action = psadpdknonzeroargdefaultaction06l85();
    }
    apply {
        tbl_psadpdknonzeroargdefaultaction06l85.apply();
    }
}

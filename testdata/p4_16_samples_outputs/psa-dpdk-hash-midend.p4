...
control IngressI(inout H hdr, inout M meta, in psa_ingress_input_metadata_t istd, inout psa_ingress_output_metadata_t ostd) {
    @name("IngressI.h") Hash<bit<32>>(PSA_HashAlgorithm_t.CRC16) h_0;
    @name("IngressI.a") action a_1() {
        meta.hash.hash = h_0.get_hash<bit<32>, tuple_0>(32w0, (tuple_0){f0 = meta.ipv4.lkp_ipv4_sa}, 32w65536);
    }
    @hidden table tbl_a {
        actions = {
            a_1();
...
control DeparserI(packet_out packet, out EMPTY_CLONE clone_i2e_meta, out EMPTY_RESUB resubmit_meta, out EMPTY_BRIDGE normal_meta, inout H hdr, in M local_metadata, in psa_ingress_output_metadata_t istd) {
    apply {
    }
}

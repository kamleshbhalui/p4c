...
control deparser(packet_out b, out EMPTY_CLONE clone_i2e_meta, out EMPTY_RESUB resubmit_meta, out EMPTY_BRIDGE normal_meta, inout Header_t h, in Meta_t local_metadata, in psa_ingress_output_metadata_t istd) {
    @hidden action psadpdktableentriesvalid107() {
        b.emit<hdr>(h.h);
    }
    @hidden table tbl_psadpdktableentriesvalid107 {
        actions = {
            psadpdktableentriesvalid107();
        }
        const default_action = psadpdktableentriesvalid107();
    }
    apply {
        tbl_psadpdktableentriesvalid107.apply();
    }
}

...
control MyIngressControl(inout headers_t hdr, inout user_meta_data_t m, in psa_ingress_input_metadata_t c, inout psa_ingress_output_metadata_t d) {
    ...
    @name("MyIngressControl.stub") table stub_0 {
        ...
        const default_action = macswp(32w1, 32w2);
        size = 1000000;
    }
    @hidden action psadpdknonzeroargdefaultaction01l69() {
        d.egress_port = (bit<32>)c.ingress_port ^ 32w1;
    }
    @hidden table tbl_psadpdknonzeroargdefaultaction01l69 {
        actions = {
            psadpdknonzeroargdefaultaction01l69();
        }
        const default_action = psadpdknonzeroargdefaultaction01l69();
    }
    apply {
        tbl_psadpdknonzeroargdefaultaction01l69.apply();
        stub_0.apply();
    }
}
...
control MyIngressDeparser(packet_out pkt, out EMPTY a, out EMPTY b, out EMPTY c, inout headers_t hdr, in user_meta_data_t e, in psa_ingress_output_metadata_t f) {
    @hidden action psadpdknonzeroargdefaultaction01l84() {
        pkt.emit<ethernet_t>(hdr.ethernet);
    }
    @hidden table tbl_psadpdknonzeroargdefaultaction01l84 {
        actions = {
            psadpdknonzeroargdefaultaction01l84();
        }
        const default_action = psadpdknonzeroargdefaultaction01l84();
    }
    apply {
        tbl_psadpdknonzeroargdefaultaction01l84.apply();
    }
}

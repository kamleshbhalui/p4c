...
control ingress(inout Headers h, inout Meta m, in psa_ingress_input_metadata_t istd, inout psa_ingress_output_metadata_t ostd) {
    @hidden action psadpdkgauntlet_function_return_cast56() {
        h.eth_hdr.eth_type = 16w255;
    }
    @hidden table tbl_psadpdkgauntlet_function_return_cast56 {
        actions = {
            psadpdkgauntlet_function_return_cast56();
        }
        const default_action = psadpdkgauntlet_function_return_cast56();
    }
    apply {
        tbl_psadpdkgauntlet_function_return_cast56.apply();
    }
}
...
control deparser(packet_out pkt, out EMPTY_CLONE clone_i2e_meta, out EMPTY_RESUB resubmit_meta, out EMPTY_BRIDGE normal_meta, inout Headers h, in Meta local_metadata, in psa_ingress_output_metadata_t istd) {
    @hidden action psadpdkgauntlet_function_return_cast76() {
        pkt.emit<ethernet_t>(h.eth_hdr);
    }
    @hidden table tbl_psadpdkgauntlet_function_return_cast76 {
        actions = {
            psadpdkgauntlet_function_return_cast76();
        }
        const default_action = psadpdkgauntlet_function_return_cast76();
    }
    apply {
        tbl_psadpdkgauntlet_function_return_cast76.apply();
    }
}
